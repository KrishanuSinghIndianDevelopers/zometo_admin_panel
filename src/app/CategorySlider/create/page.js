...
export default function CreateCategorySlider() {
  ...
  const fetchMainCategories = async (userObj) => {
    try {
      ...
      if (isAdminUser) {
        // ADMIN: Show ALL main categories (no vendor filter)
        categoriesQuery = query(
          collection(db, 'categories'),
          where('isMainCategory', '==', true)
        );
      } else {
        // VENDOR: Show only their own main categories
        categoriesQuery = query(
          collection(db, 'categories'),
          where('isMainCategory', '==', true),
              ...
              {user && (
                <small className="text-info">
                  {user.role === 'admin' || user.role === 'main_admin' 
                    ? 'Viewing all vendor categories' 
                    : 'Viewing your categories only'}
                </small>
              )}
            </div>
                ...
                <div className="card-body p-4">
                  ...
                  <div className="small">
                    <div className="d-flex justify-content-between mb-3 pb-2 border-bottom">
                      <span className="text-muted">Available Categories:</span>
                      <span className="fw-medium text-end">{categories.length}</span>
                    </div>
                    <div className="d-flex justify-content-between mb-3 pb-2 border-bottom">
